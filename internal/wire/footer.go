...
func ReadFooter(r io.ReaderAt, fileSize int64, expectedMagic [8]byte) (cktype.Footer, error) {
	...
	if _, err := r.ReadAt(buf[:], off); err != nil {
		return cktype.Footer{}, fmt.Errorf("reading footer: %w", err)
	}
	...
	if f.Magic != expectedMagic {
		return cktype.Footer{}, fmt.Errorf("%w: footer magic got %q, want %q", cktype.ErrBadMagic, f.Magic[:], expectedMagic[:])
	}

	if f.OverflowOffset != 0 && f.OverflowOffset >= uint64(fileSize)-FooterSize {
		return cktype.Footer{}, fmt.Errorf("%w: overflow_offset %d beyond file", cktype.ErrOffsetOutOfBounds, f.OverflowOffset)
	}
